/***************************************************************************
	...
	$Id: wegstructure.cpp 375 2008-10-28 14:47:15Z neoneurone $
 ***************************************************************************/
...
// IF this is an electric plant THEN turn on the electricity bit
	if (enumStructCode == OC_STRUCTURE_EPLANT_COAL) {
        ...
        if (enumStructCode == OC_STRUCTURE_EPLANT_WIND) {
		Set( OC_STRUCTURE_E );
	} else
        if (enumStructCode == OC_STRUCTURE_EPLANT_OIL) {
		Set( OC_STRUCTURE_E );
	} else
		   ...
		   // Then turn the E bit on
			if (pMain!= NULL) {
				if (pMain->GetCode() == OC_STRUCTURE_EPLANT_COAL)
					...
					this->Set( OC_STRUCTURE_E );
                                else if (enumStructCode == OC_STRUCTURE_EPLANT_WIND)
                                        this->Set(OC_STRUCTURE_E);
                                else if (enumStructCode == OC_STRUCTURE_EPLANT_OIL)
                                        this->Set(OC_STRUCTURE_E);
			}
			break;
