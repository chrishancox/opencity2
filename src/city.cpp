...
// Load the buttons used by the status bar
	...
	_apbtnCurrentTool[OC_TOOL_EPLANT_WIND]
		= new GUIButton( GUIBUTTON_POSITION_TOOL, ocDataDirPrefix( "graphism/gui/status/power_plant_wind" ));
	_apbtnCurrentTool[OC_TOOL_EPLANT_NUCLEAR]
        ...
        _apbtnCurrentTool[OC_TOOL_EPLANT_OIL]
		= new GUIButton( GUIBUTTON_POSITION_TOOL, ocDataDirPrefix( "graphism/gui/status/power_plant_oil" ));
...
// block all the sim threads while modifying the game datas
	...
	case OC_TOOL_EPLANT_NUCLEAR:
        ...
        case OC_TOOL_EPLANT_WIND:
		if ((enumErrCode = _apLayer[ _eCurrentLayer ]->
			BuildStructure(
				_uiMapW1, _uiMapL1, _uiMapW2, _uiMapL2,
				OC_STRUCTURE_EPLANT_WIND, cost )) == OC_ERR_FREE) {
			_pMSim->AddStructure( _uiMapW1, _uiMapL1, _uiMapW2, _uiMapL2, Simulator::OC_ELECTRIC );
			gVars.gpAudioMgr->PlaySound( OC_SOUND_EPLANT );
		}
		break;
                
        case OC_TOOL_EPLANT_OIL:
		if ((enumErrCode = _apLayer[ _eCurrentLayer ]->
			BuildStructure(
				_uiMapW1, _uiMapL1, _uiMapW2, _uiMapL2,
				OC_STRUCTURE_EPLANT_OIL, cost )) == OC_ERR_FREE) {
			_pMSim->AddStructure( _uiMapW1, _uiMapL1, _uiMapW2, _uiMapL2, Simulator::OC_ELECTRIC );
			gVars.gpAudioMgr->PlaySound( OC_SOUND_EPLANT );
		}
		break;
...
// the user clicked on the eLectric toolcircle
	...
	switch (uiObject) {
		...
		case 4:  // set tool for building coal power plant
			...
			break;
                case 5:  // set tool for building wind power plant
			_SetCurrentTool( OC_TOOL_EPLANT_WIND );
			break;
                case 6:  // set tool for building wind power plant
			_SetCurrentTool( OC_TOOL_EPLANT_OIL );
			break;
	...
	if (tcode != _eCurrentTool) {
		...
		*/
			...
			case OC_TOOL_EPLANT_NUCLEAR:
				...
				break;
                        case OC_TOOL_EPLANT_WIND:
				scode = OC_STRUCTURE_EPLANT_WIND;
				break;
                        case OC_TOOL_EPLANT_OIL:
				scode = OC_STRUCTURE_EPLANT_OIL;
				break;
			case OC_TOOL_PARK:
